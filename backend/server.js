...
async function ensureSchema() {
  ...
  // PG 9.6+ supports ADD COLUMN IF NOT EXISTS — no information_schema dance needed.
...
app.get('/api/health', async (_req, res) => {
  try {
    ...
    res.json({ status: 'ok', database: rows[0]?.ok === 1 ? 'connected' : 'unknown', driver: 'postgres' })
  } catch (error) {
...
app.get('/api/products/:id', async (req, res) => {
  try {
    ...
    const [specRows] = await pool.query(
      `SELECT spec_name, spec_value FROM product_specs WHERE product_id = ? ORDER BY id ASC`,
      [id]
    )
...
app.post('/api/orders', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const [orderInserted] = await connection.query(
      'INSERT INTO orders (user_id, customer_name, customer_email, customer_phone, total_amount, status) VALUES (?, ?, ?, ?, ?, ?) RETURNING id',
...
app.put('/api/admin/users/:id', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    if (!isValidEmail(email)) return res.status(400).json({ message: 'Некоректний email.' })
    if (!isValidPhone(phone)) return res.status(400).json({ message: 'Некоректний номер телефону.' })
...
app.post('/api/admin/products', authMiddleware, adminMiddleware, async (req, res) => {
  ...
  try {
    ...
    const [inserted] = await connection.query(
      'INSERT INTO products (name, brand, scale, price, description, image_url, stock) VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id',
      [
        name.trim(),
        brand.trim(),
        scale.trim(),
    ...
    for (const spec of specifications) {
      if (spec?.name && spec?.value) {
        await connection.query(
          ...
          [newId, String(spec.name).trim(), String(spec.value).trim()]
        )
      }
    }

    await connection.commit()
    res.status(201).json({ message: 'Товар додано.', id: newId })
  } catch (error) {
    await connection.rollback()
    res.status(500).json({ message: error.message })
  } finally {
  ...
  try {
    const [rows] = await pool.query(`
      SELECT r.id, r.product_id, p.name AS product_name,
             COALESCE(u.name, r.author_name, 'Користувач') AS author,
...
app.put('/api/admin/reviews/:id', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [Number(rating || 5), String(comment).trim(), req.params.id]
    )
...
ensureSchema()
  .then(() => {
    app.listen(PORT, () => {
      console.log(`Postgres backend started on http://localhost:${PORT}`)
    })
  })
  .catch((error) => {
